...
- 2025‑09‑05: Phase H3 — Extracted shared layout styles to `css/components.css` (container-centered, app layout, universal sidebar shell, mobile bottom nav, mobile nav trigger). Removed `company-card-fixed.css` includes from non‑company pages. Files: css/components.css, css/company-card-fixed.css, index.html, about.html, legal.html, documentation/*
- 2025‑09‑05: Universals polish — Sidebar close handle centered mid-edge on all pages; Recent Companies styles shared; universal footer adapts for About/Legal; About/Legal now load Fonts + Firebase, fixing header search and typography. Files: css/components.css, components/universal-footer.html, about.html, legal.html, js/universal-sidebar.js
- 2025‑09‑05: Universal sidebar — Self-hydrates markup and centralizes collapse/close wiring (delegated handler). Removed per-page sidebar fetch/wiring; desktop close arrow now works consistently across pages. Files: js/universal-sidebar.js, index.html, about.html, legal.html, js/company-card-complete-fix.js, documentation/*
- 2025‑09‑05: Analytics — Added GA4 initialization via `js/services/AnalyticsService.js` (centralized, reversible). Logs `page_view` on load for all pages, with optional `?ga_debug=1` flag. Files: js/services/AnalyticsService.js, index.html, company-card-fixed.html, about.html, legal.html, documentation/*
- 2025‑09‑05: Header consistency — Removed company overrides; scoped company touch‑friendly button rule to avoid inflating universal header buttons; stabilized avatar sizing across pages; toned down avatar hover. Files: css/company-card-fixed.css, components/universal-header.html
- 2025‑09‑05: Phase H3 — Shared pills/badges extracted to `css/components.css` (`.tier-badge-mini`, `.ch-tier-badge`, `.tier-badge-compact`); duplicates removed from page CSS. Files: css/components.css, css/index.css, css/company-card-fixed.css, documentation/*
- 2025‑09‑05: Phase H2 start — Lazy‑loaded Detailed Financials on company page. `js/views/tables.js` now loads on first visibility with a skeleton; legacy calls proxy through bootstrap. Files: js/company-boot.js, company-card-fixed.html, documentation/*
- 2025‑09‑05: Input theming — Moved shared input/select styling (`.sidebar-input`) to `css/components.css` for consistent dark/light theme across pages; fixed index sidebar filter and per-page dropdown appearances. Files: css/components.css, css/company-card-fixed.css
- 2025‑09‑05: Payload reduction — Removed unused Chart.js include; earnings chart uses Canvas 2D and is drawn lazily on first visibility (along with mini price chart). Files: company-card-fixed.html, js/company-boot.js, documentation/*
- 2025‑09‑05: Phase H1 — Company bootstrap added (`js/company-boot.js`) and score-cards wiring moved to bootstrap (tabs, sticky offsets, mobile carousel). No behavior changes. Docs updated (file-structure + TODO H1 progress). Files: js/company-boot.js, company-card-fixed.html, js/company-card-complete-fix.js, documentation/*
